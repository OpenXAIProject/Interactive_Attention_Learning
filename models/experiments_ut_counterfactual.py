...
def Find_influential_training_input(model, 
    ...
    chosen_training_indices = model.get_ut_from_trainingpoints(num_to_choose=num_to_choose_train, dropout=True,)
    ...
    np.save(os.path.join(train_dir, 'chosen_training_indices.npy'), chosen_training_indices)

    #Sampling feature contribution
    f_samples = np.zeros((num_sampling, \
                        chosen_training_indices.shape[0], \
    ...
    inputs = model.data_sets.train.x[chosen_training_indices]
    labels = model.data_sets.train.labels[chosen_training_indices]
    masked_input = model.data_sets.train.x[chosen_training_indices]
    masked_labels = model.data_sets.train.labels[chosen_training_indices]
    feature_importance_box = np.zeros([masked_input.shape[0], masked_input.shape[1], masked_input.shape[2]])
    for feature_ in range(masked_input.shape[2]):
        masked_input[:, :, feature_] = 0
        idv_feature_importance = model.get_feature_importance_with_counterfactual(inputs,
                                                                                  labels,
                                                                                  masked_input,
                                                                                  masked_labels
                                                                                 )
        
        for step_ in range(masked_input.shape[1]):
            feature_importance_box[:,step_,feature_] = idv_feature_importance
    ...
    samples = {
                ...
                "FeatureImportance": feature_importance_box,
               }

    result_folder = os.path.join('./annotation/Samples')
    ...
    print('IF-Counterfactor_Training file has been created.')
    return samples
...
def further_stage_Find_influential_training_input(model, 
    ...
    with open(os.path.join(result_folder, stage+task+'_Samples_ut_Counterfactual.pickle'), 'wb') as handle:
        pickle.dump(samples, handle, protocol=pickle.HIGHEST_PROTOCOL)
